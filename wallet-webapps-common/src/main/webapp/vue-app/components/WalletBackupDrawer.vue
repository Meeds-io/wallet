    ...
    walletBackedUp() {
      setWalletBackedUp()
        .then(() => {
          ...
          this.$root.$emit('show-alert', {type: 'error',message: String(error)});
        })
        .finally(() => {
          this.loading = false;
        });
      ...
      try {
        if (unlocked) {
          ...
          if (!wallet || !wallet.privateKey) {
            this.$root.$emit('show-alert', {type: 'error',message: this.$t('exoplatform.wallet.error.walletNotFound')});
            return;
          }
          this.loading = false;
          this.walletPrivateKey = wallet.privateKey;
        } else {
          this.$root.$emit('show-alert', {type: 'error',message: this.$t('exoplatform.wallet.warning.wrongPassword')});
        }
      } finally {
        this.loading = false;
        if (unlocked) {
