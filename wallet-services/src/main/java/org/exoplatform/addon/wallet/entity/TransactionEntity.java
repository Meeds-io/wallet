...
@NamedQueries({
    @NamedQuery(name = "WalletTransaction.countReceivedContractAmount", query = "SELECT SUM(tx.contractAmount) FROM WalletTransaction tx WHERE tx.networkId = :networkId AND (tx.contractAddress = :contractAddress OR tx.toAddress = :contractAddress) AND tx.toAddress = :address AND (tx.contractMethodName = 'reward' OR tx.contractMethodName = 'initialize' OR tx.contractMethodName = 'transfer' OR tx.contractMethodName = 'transferFrom') AND tx.createdDate > :startDate AND tx.createdDate < :endDate"),
