...
public class KeccakDigest
    ...
    while (i < databitlen) {
      if ((bitsInQueue == 0) && (databitlen >= rate) && (i <= (databitlen - rate))) {
        ...
        for (j = 0; j < wholeBlocks; j++) {
          ...
          // displayIntermediateValues.displayBytes(1, "Block to be absorbed",
          // curData, rate / 8);
  ...
  private void padAndSwitchToSqueezingPhase() {
    ...
    // displayIntermediateValues.displayText(1, "--- Switching to squeezing
    // phase ---");
    ...
    // displayIntermediateValues.displayBytes(1, "Block available for
    // squeezing", dataQueue, bitsAvailableForSqueezing / 8);
  ...
  private void keccakPermutation(byte[] state) {
    ...
    // displayIntermediateValues.displayStateAsBytes(1, "Input of permutation",
    // longState);

    keccakPermutationOnWords(longState);

    // displayIntermediateValues.displayStateAsBytes(1, "State after
    // permutation", longState);
  ...
  private void keccakPermutationOnWords(long[] state) {
    ...
    // displayIntermediateValues.displayStateAs64bitWords(3, "Same, with lanes
    // as 64-bit words", state);

    for (i = 0; i < 24; i++) {
      // displayIntermediateValues.displayRoundNumber(3, i);

      theta(state);
      // displayIntermediateValues.displayStateAs64bitWords(3, "After theta",
      // state);

      rho(state);
      // displayIntermediateValues.displayStateAs64bitWords(3, "After rho",
      // state);
      ...
      chi(state);
      // displayIntermediateValues.displayStateAs64bitWords(3, "After chi",
      // state);

      iota(state, i);
      // displayIntermediateValues.displayStateAs64bitWords(3, "After iota",
      // state);
    }
  }
    ...
    for (int x = 0; x < 5; x++) {
      for (int y = 0; y < 5; y++) {
        ...
        A[index] = ((KeccakRhoOffsets[index] != 0)
                                                   ? (((A[index]) << KeccakRhoOffsets[index])
                                                       ^ ((A[index]) >>> (64 - KeccakRhoOffsets[index])))
                                                   : A[index]);
      }
    }
  }
